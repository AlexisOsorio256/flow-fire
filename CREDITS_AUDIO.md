...
- `assets/audio/source/sonniss_gdc2016_glock18c_1m.wav`: fichero original del bundle, ruta interna `Sonniss.com - GDC 2016- Game Audio Bundle/Pole Position Production - Glock 18c/Glock_18_1m_left_off_axis_MKH416_clean_Six_shots_x_1.wav`, 96 kHz / 16 bits / mono, 3,7333 s (md5 `cc9ded0c7db77e04d9196c2899d7bab2`; convertido de los 24 bits originales a PCM canónico porque el editor no importa WAV extensibles/24-bit y los marcaba como error: mismo audio, sin muestras al ras nuevas). Es la fuente de la que se cortan `shot_1..5.wav`. Los seis disparos que anuncia el nombre del fichero son los seis ataques medidos arriba: cuadra.
...
- Cada `shot_N` lleva fusionado el cuerpo grave de una Beretta 93R 9 mm a 1 m (`assets/audio/source/beretta93r_body_excerpt.wav`: recorte 0.55-0.95 s de `Beretta_93R_9mm_1m_left_on_ground_DPA4062_clean_Triple_shots_x_1.wav`, 96 kHz / 16 bits / mono, tiro nº 3, 0 muestras al ras; los otros dos tiros del triple están recortados y se rechazan). Alineación por pico de muestra (<1 ms: un solo evento) con el cuerpo 4 dB bajo el pico del blast — medido: el grave sube de ~7% a ~11% sin tocar el crack ni el ataque. La misma mezcla a más nivel tapaba el crack; aislar el grave con filtro lo retrasaba (probado y descartado).
...
- `assets/audio/source/g36c_mag_in_out_excerpt.wav`: recorte 11.28-11.70 s de la toma original, 96 kHz / 16 bits / mono (PCM canónico, mismo motivo que arriba). Es la fuente versionada de la que se cortan `handling`/`magin`/`magout` con `tools/process_audio.sh` (`process_mag`).
- `handling.wav`: **roce de manos** (100 ms) del pre-gesto del mismo ciclo — Sonniss #GameAudioGDC Bundle 2016, misma licencia. Foley del Inspect (abrazo del arma).
